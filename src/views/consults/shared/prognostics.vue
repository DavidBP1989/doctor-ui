...
export default {
    ...
    methods: {
        setPrognostics() {
            ...
            if (this.evolution) this.listOfNewValues.push('Reservado a evolución')
            if (this.goodSelected.length > 0) {
                if (this.goodSelected.includes(0))
                    this.listOfNewValues.push('Bueno para la vida')
                if (this.goodSelected.includes(1))
                    this.listOfNewValues.push('Bueno para la función')
            }
            if (this.badSelected.length > 0) {
                if (this.badSelected.includes(0))
                    this.listOfNewValues.push('Malo para la vida')
                if (this.badSelected.includes(1))
                    this.listOfNewValues.push('Malo para la función')
            }
        }
    },
    watch: {
